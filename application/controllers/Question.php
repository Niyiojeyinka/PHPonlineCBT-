...
public function freedom_test($slug = null)
{
  $v_slug = "question/freedom_test";
    $this->board_model->insert_view($v_slug);
    ...
    $i =1;
      ...
      $i =2;
        ...
        $i =3;
          ...
          if(isset($_SESSION['subject_ids']))
          {
            unset($_SESSION['subject_ids']);
          }

  $subject_holder = json_decode($this->users_model->get_user_by_id()['subjects'])[$i];


  $_SESSION['running'] = $this->input->post('time');

//$acct_type ="free";
$condition_array  = array('subject' => $subject_holder,
 'status' => 'published','account_type' => $acct_type);
$out_put = $this->question_model->get_questions($condition_array);
$subject_items = randomize($out_put);
//get english question out of available randoms
 $subject_items = array_slice($subject_items,1,$this->input->post('no'));
$_SESSION['subject_ids'] = $subject_items;
...
array('16','17'));*/
         ...
         $data['items'] =   json_decode($this->users_model->get_user_by_id()['subjects']);
         $data['user_details'] = $this->users_model->get_user_by_id();
        $_SESSION['subject']  = $subject_holder;

        $this->load->view('common/headmeta_view',$data);
            $this->load->view('user/common/users_nav_view',$data);
            $this->load->view('common/header_view',$data);
             $this->load->view('user/pre_freedom_test_view',$data);
        $this->load->view('common/footer_view',$data);
    }//close form validation true block
else{
...
public function freedom_question_page()
{
if(!isset($_SESSION['start_time']))
{
  $_SESSION['start_time'] = time();
  ...
  if(isset($_SESSION['timeout']) || isset($_SESSION['submit']))
  {
    show_page('question/freedom_submit');
  }

  $v_slug = "question/freedom_question_page";
    $this->board_model->insert_view($v_slug);
...
*/
  ...
  $index_to_use = $_SESSION["question_index"]['freedom'] - 1;
}


$question_id = $_SESSION['subject_ids'][$index_to_use];
$data['count_sub'] =count($_SESSION['subject_ids']);
...
$question_id = $_SESSION['subject_ids'][$bindex];
 $_SESSION["question_index"] = $data['count_sub'] -1;
}



$data['user_details']['acct_type'] = "free";
$condition_array  = array('subject' => $_SESSION['subject'],'id' =>
 $question_id,'account_type' => $data['user_details']['acct_type']);
$data['question'] = $this->question_model->get_question($condition_array);


$data["q_id"] = $question_id;
  $data["q_index"] = array_search($question_id,$_SESSION['subject_ids']);
  //index of question_id in an array;
          ...
          $this->load->view('common/headmeta_view',$data);
              $this->load->view('user/common/users_nav_view',$data);
              $this->load->view('common/header_view',$data);
               $this->load->view('user/freedom_question_view',$data);
          $this->load->view('common/footer_view',$data);
 ...
 public function freedom_question_act()
{

    if(!isset($_SESSION['user_answers']['freedom']))
    {
      $_SESSION['user_answers']['freedom'] =[];
    }


        if(!isset($_SESSION['correct_answers']['freedom']))
        {
          $_SESSION['correct_answers']['freedom'] =[];
        }


                if(!isset($_SESSION['question_m_id']['freedom']))
                {
                  $_SESSION['question_m_id']['freedom'] =[];
                }


                                if(!isset($_SESSION['question_number']['freedom']))
                                {
                                  $_SESSION['question_number']['freedom'] =[];
                                }
...
if($diff_t >= (($_SESSION['running'] * 60) - 5))
{
  ...
  $_SESSION['stopped_time'] = time();
            ...
            if(!isset( $_SESSION["question_index"]['freedom']))
              {
                 $_SESSION["question_index"]['freedom'] =1;
                                                                      }
...
if(!empty($_POST['option']))
{
              $condition_array  = array('subject' => $_SESSION['subject'],'id' => $q_id);
               $data['question'] = $this->question_model->get_question($condition_array);
           $correct_answer = $data['question']['answer'];
           $user_answer = $_POST['option'];
$_SESSION['user_answers']['freedom'][$q_index] = $user_answer;
$_SESSION['correct_answers']['freedom'][$q_index] = $correct_answer;
$_SESSION['question_m_id']['freedom'][$q_index] = $q_id;
if(!in_array($q_index +1,$_SESSION['question_number']['freedom']))
{
array_push($_SESSION['question_number']['freedom'],$q_index +1);
...
if(isset($_POST['next']))
    {
$_SESSION['count_sub_np'] = $_POST['total_no_q'];
$_SESSION["question_index"]['freedom'] =$_SESSION["question_index"]['freedom'] + 1;

//check here
    show_page("question/freedom_question_page");
...
if(isset($_POST['qno']))
{
    ...
    $_SESSION["question_index"]['freedom'] = $_POST['qno'];
...
public function freedom_submit()
{
/*
  echo var_dump($_SESSION['user_answers']['freedom']);
  echo var_dump($_SESSION['correct_answers']['freedom']);
*/
$v_slug = "question/freedom_submit";
  $this->board_model->insert_view($v_slug);


$_SESSION['freedom_score'] = 0;

  if(count($_SESSION['user_answers']['freedom']) !=0)
    {
for($i =0;$i<count($_SESSION['user_answers']['freedom']);$i++)
{
if($_SESSION['user_answers']['freedom'][$i] ==
$_SESSION['correct_answers']['freedom'][$i])
{
  $_SESSION['freedom_score'] = $_SESSION['freedom_score'] + 1;
}
...
//save result to db later here
      ...
      $this->load->view('common/headmeta_view',$data);
          $this->load->view('user/common/users_nav_view',$data);
          $this->load->view('common/header_view',$data);
          $this->load->view('user/freedom_result_view',$data);
      $this->load->view('common/footer_view',$data);
...
//function
public function freedom_correction($offset =NULL){

  $v_slug = "question/freedom_correction";
    $this->board_model->insert_view($v_slug);
...
$data['subject_items'] =   json_decode($this->users_model->get_user_by_id()['subjects']);
        ...
        $this->load->view('common/headmeta_view',$data);
            $this->load->view('user/common/users_nav_view',$data);
            $this->load->view('common/header_view',$data);
            $this->load->view('user/freedom_correction_view',$data);
        $this->load->view('common/footer_view',$data);
