...
//split bulk upload best for text question only



public function upload_split_batch()
{
...
$name = str_replace(" ", "_", $this->input->post('subject'))."_".$this->input->post('year')."_".$this->input->post('paper_type')."_".$this->input->post('starting_num')."-".$this->input->post('stopping_num')."_".$this->input->post('exam_type');
        $config['upload_path'] = './assets/batch/split/';
$config['allowed_types'] = 'xml';
$config['max_size']     = '90000';
$config['file_name']     = $name."_split.xml";
  ...
  if(!$this->form_validation->run() )
    ...
    $this->load->view('/admin/header_view',$data);
      ...
      $this->load->view('admin/sidebar_view',$data);
    $this->load->view('admin/questions/question_split_batch_view',$data);
      $this->load->view('admin/footer_view');

    }else{
        $this->upload->do_upload('question_file');
        
        if($this->admin_question_model->save_split_bulk_question($this->upload->data("file_name"),$this->upload->data("file_ext")))
        {
...
$_SESSION['action_status_report'] = "<span class='w3-text-red'>Unknown Error Occurred PLs Try Again</span>";
$this->session->mark_as_flash('action_status_report');
show_page('admin_question/upload_split_batch');
...
public function save_split_question_todb($id)
{


$q_det = $this->admin_question_model->get_bulk($id);

$file_link = base_url('assets/batch/split/'.$q_det["file_name"]);
...
$condition_array = array('subject' => $q_det['subject'],'year' => $q_det['year'],
'paper_type'  => $q_det['paper_type'],'question_number'  => $question->questionnumber  );
...
//sorry no mvc here
  if($question->optione == "NULL" || $question->optione == "" )
  {
    $alt_optione = NULL;
  }else{
        $alt_optione = $question->optione;
...
'subject' => $q_det['subject'],/**/
  'year' => $q_det['year'],/**/
'answer' => NULL,/**/
'account_type' =>  $q_det['account_type'],/**/
'option_a' =>  $question->optiona,
'option_b' => $question->optionb,
'option_c' => $question->optionc,
'option_d' => $question->optiond,
...
if( $this->db->insert('questions',$question))
{
 ...
 $data['action_status_report'] .=  "<span class='w3-text-green w3-margin w3-small'>subject  question number ".$question->questionnumber." saved to the database Successfully</span> <br>";
 ...
 $data['action_status_report'] .=  "<span class='w3-text-red w3-margin w3-small'>subject  question number ".$question->questionnumber." failed to save to the database Successfully</span><br> ";
...
public function upload_split_answer()
{
       ...
       $this->form_validation->set_rules("year","Year","required");
       $this->form_validation->set_rules("paper_type","Paper Type","required");
       $this->form_validation->set_rules("stopping_num","Stopping Number","required");
...
$config['allowed_types'] = 'xml';
$config['max_size']     = '90000';
$config['file_name']     = $name."_ANSWER.xml";
  ...
  if(!$this->form_validation->run() )
    ...
    $this->load->view('/admin/header_view',$data);
      ...
      $this->load->view('admin/sidebar_view',$data);
    $this->load->view('admin/questions/question_split_answer_view',$data);
      $this->load->view('admin/footer_view');

    }else{
        $this->upload->do_upload('answer_file');
...
$_SESSION['action_status_report'] = "<span class='w3-text-red'>Unknown Error Occurred PLs Try Again</span>";
$this->session->mark_as_flash('action_status_report');
show_page('admin_question/upload_split_answer');
...
public function save_split_answer_todb($id)
{
...
$xml=simplexml_load_file($file_link) or die("Error: Cannot create object");
foreach($xml->children() as $answer) { 


//check if question exist already

$condition_array = array('subject' => $q_det['subject'],'year' => $q_det['year'],
'paper_type'  => $q_det['paper_type'],'question_number'  => $answer->ano  );

$outp = $this->admin_question_model->get_questions($condition_array);
if(empty($outp))
{
  
 $data['action_status_report'] .= "<div class='w3-text-red w3-margin w3-small'> question number ".$answer->ano." Not Exist</div> ";
...
'answer' => $answer->canswer,
 'time' => time()/**/
);


if( $this->db->update('questions',$question,array('paper_type'  => $q_det['paper_type'],'question_number'  => $answer->ano,'subject' => $q_det['subject'],'year' => $q_det['year'] )))
{




 $data['action_status_report'] .=  "<span class='w3-text-green w3-margin w3-small'>QUESTION ANSWER ".$answer->ano." saved to the database Successfully</span> <br>";
 ...
 $data['action_status_report'] .=  "<span class='w3-text-red w3-margin w3-small'> ANSWER failed to save to the database Successfully</span><br> ";
